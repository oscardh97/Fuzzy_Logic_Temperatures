...
var compare = function(avgValues, actualValue, type) {
    ...
    $("#tableValues" + (type === 1 ? "Day" : type === 2 ? "Month" : "Year")).empty().append(tr);
...
var analizeData = function(data) {
    ...
    console.log("Day", compare({
        avgMax: parseFloat(data.weather[0].maxtempC),
        avgMin: parseFloat(data.weather[0].mintempC)
    }, data.current_condition[0].FeelsLikeC, 1));

    console.log("Month", compare(yearData.month, data.current_condition[0].FeelsLikeC, 2));
    console.log("Year", compare(yearData.year, data.current_condition[0].FeelsLikeC, 3));

    $("#valActual").html(data.current_condition[0].FeelsLikeC + " C");
    $("#valMinD").html(parseFloat(data.weather[0].mintempC).toFixed(2) + " C");
    $("#valMaxD").html(parseFloat(data.weather[0].maxtempC).toFixed(2) + " C");
